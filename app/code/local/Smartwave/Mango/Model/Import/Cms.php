...
class Smartwave_Mango_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
	...
	public function importCms($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('mango')->__("Can't get the data file for import cms blocks/pages: %s", $xmlPath)
                );
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('mango')->__('%s item(s) was(were) imported.', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('mango')->__('No items were imported')
				);
			}
